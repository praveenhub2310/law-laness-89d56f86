-- Fix the column ambiguity issue in the function
...
DECLARE
  ...
  assigned_lawyer_id uuid := lawyer_id;
BEGIN
  ...
  ELSE
    ...
    AND (projects.lawyer_id IS NULL OR projects.lawyer_id != assigned_lawyer_id);
  END IF;
  
  RAISE NOTICE 'Successfully assigned lawyer % to client %', assigned_lawyer_id, client_email;
END;
$$;