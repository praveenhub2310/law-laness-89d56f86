...
const ProjectsManagement = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      filterable: true,
      filterOptions: ['Personal Injury', 'Real Estate', 'Corporate Law', 'Family Law', 'Criminal Law'],
      className: 'min-w-[120px]',
      render: (value: string) => (
  ...
  const fields = [
    ...
    { 
      ...
      options: ['Personal Injury', 'Real Estate', 'Corporate Law', 'Family Law', 'Criminal Law'],
      required: true 
    },
  ...
  const handlePreview = (item: any) => {
    // Generate a PDF-like content structure
...
Report Type: Case Summary Report
    `;
    
    // Create a more structured document format
    const blob = new Blob([content], { type: 'application/msword' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${item.id}_Case_Report.doc`;
  ...
  return (
    <div className="p-3 sm:p-6">
      <DataTable
        ...
        data={data}
        fields={fields}
        searchPlaceholder="Search cases by title, client, or case ID..."
        onAdd={addItem}
        onEdit={updateItem}
        onDelete={deleteItem}
        onExport={exportData}
